...
Clusterizer::Clusterizer(double timeCut, double timeMin, double timeMax, double gradientCut, bool doEnergyGradientCut, double thresholdSeedE, double thresholdCellE) :
    mSeedList(), mDigitMap(), mCellMask(),
    mTimeCut(timeCut), mTimeMin(timeMin), mTimeMax(timeMax), mGradientCut(gradientCut),
    mDoEnergyGradientCut(doEnergyGradientCut), mThresholdSeedEnergy(thresholdSeedE), mThresholdCellEnergy(thresholdCellE)
{
}
...
void Clusterizer::getClusterFromNeighbours(std::vector<Digit*>& clusterDigits, int row, int column)
{
  ...
  for(int dir=0; dir<4; dir++)
  {
    ...
    if( (row+rowDiff < 0) || (row+rowDiff >= NROWS) ) continue;
    if( (column+colDiff < 0) || (column+colDiff >= NCOLS) ) continue;
